# First Voice v1.1 Checkpoint

**Project:** MerrinLab Ultimate Synth  
**Checkpoint:** First Voice v1.1  
**Status:** Stable playable three-oscillator browser baseline before the next feature  
**Scope:** Documentation checkpoint only  

## 1. Purpose

This checkpoint records the current stable playable state of the browser preview after VCO 3 was added and tested.
...
The purpose is to freeze the current working three-oscillator baseline before adding pitch Sample & Hold, external input, patch routing, sync, PWM modulation, MIDI, or plugin work.
...
- Panic Stop silences and stops VCO 1, VCO 2, VCO 3, selected noise, LFO 1, S&H offset/timer, LFO 2, envelope/VCA path, Repeat Gate timer, tremolo path, and output path.
...
1. **Controlled oscillator detune helper / tuning notes only**
   - Documentation or small UI helper only.
   - Do not change the oscillator engine.

2. **S&H to pitch as a separate optional default-off issue**
   - Only consider after the three-oscillator path remains stable.
...
```text
First Voice v1.1 is the stable playable three-oscillator baseline before the next feature.
```
